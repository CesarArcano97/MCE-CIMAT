---
title: "Tarea-01-Inferencia-Estadística"
...
# Problema 3: Para el siguiente ejercicio es necesario programar en R.

## Escriba un programa en R que reproduzca las gráficas de las funciones de distribución acumulada y masa de la distribución uniforme que aparecen en las notas del curso. Las gráficas deben verse similares a las de las noras del curso. 
...
```
Al final podemos ver cómo a medida que añadimos más parámetros, la función F(X), es decir, la función CDF comienza a verse como si fuera una línea recta (siendo más precisos, como la identidad). Esto tiene sentido, ya que a medida que añadimos más y más segmentos, estos irán siendo más y más pequeños, volviendose casi imperceptibles. 

## Lea la documentación de R, o en cualquier otra fuente confiable, la explicación de la función "sample(x, size, replace=FALSE, prob=NULL)". 

De acuerdo con [la página](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/sample) de la documentación de R, la función _sample_ hace lo siguiente:

> "Sample" toma una muestra del tamaño especificado de los elementos de 'x', con o sin reemplazo. 

La recomendación es utilizarlo de la siguiente manera: "sample(x, size, replace = FALSE, prob = NULL)". 

Podemos realizar un ejemplo. Supongamos que nos dan un vector con 20 números y queremos tomar una muestra aleatoria de 3 de esos números. Para ello, podemos hacer lo siguiente en R:
...
También podemos hacer un ejemplo de un evento con muestras con remplazo, es decir, cuando un elemento de la población estudiada vuelve de neuvo a la población para tener, nuevamente, la probabilidad de ser seleccionado. Esto sucede, por ejemplo, cuando seleccionas una carta de un maso de baraja inglesa y vuelves a colocarla en algún lugar del maso.  

Con la función _sample_, podríamos hacer algo sencilo, de esta manera:
...
Notaremos que, muy probablemente, algún elemento se repita en nuestro muestreo. 

De igual forma, podríamos hacer un muestreo con probabilidades específicas. Un ejemplo se vería así:
...
Ahora, podemos hacerla también con la condición de reemplazo:
```{R}
...
## Usando la función _sample_, simule una muestra de tamaño 10,000 de la distribución U(1, 2, 3, ... , 10). Fijando una semlla en 13, muestre los resultados de la simulación en una tabla de frecuencia y calcule la media y la varianza. (_Hint: usar la función table_). 
...
Creo que es importante señalar que en este problema fue necesario utilizar reemplazo para poder hacer el conteo de las veces que se repite un número en el muestro de 10,000. 

Además de lo anterior, podemos ir un poco más allá, y aprovechar para graficar lo anterior:
...
# Problema 4
### Inciso a): Usando la función _sample_, simula 10 lanzamiento de una moneda equilibrada y cuenta el número de águilas que se obtienen. Repite el proceso por $10^4$ veces y muestra los primeros tres resultados. Grafica las frecuencias del número de águilas obtenidas en los $10^4$ experimentos. También grafica las proporciones del número de águilas obtenidas. 

Antes que nada, lo que haremos será intentar comprender el problema. Lo que se nos está pidiendo es simular 10 lanzamientos de una moneda equilibrasa. Esto significa que la moneda no está "manipulada", es decir, no tiene ningun arreglo de tal manera que haya una mayor probabilidad de que caiga águila o sol. Después de los 10 intentos, contaremos cuántas veces cayó águila. Posteriormente, se nos pide repetir el proceso durante $10^4$ veces y mostrar los primeros tres resultados. 
...
```
Para este código utilizamos rbinom(repeticiones, size = pruebas, prob = 0.5): esta función genera una muestra aleatoria de experimentos binomiales porque se ajusta al lanzamiento de monedas. Es decir, la distribución binomial es adecuada para representar la cantidad de éxitos en una serie de experiemtnos que tienen solo dos resultados: fracaso o éxito, cara o cruz, sol o águila. 

En este caso, rbinom cuenta directamente el número de éxitos (águilas) en cada experimento, facilitiando el proceso, pues alternativamente podríamos haber utilizado replicate y sample para resolver este caso. Además, la repeticiones es el número o cantidad de experimentos, size = pruebas es el número de lanzamientos por experimento, y prob = 0.5 es la probabilidad de obtener "águila" por lanzamiento.
...
```
Aquí utilizamos la función table para calcular la frecuencia de cada posible resultado y luego la diividimos por la cantidad total de repeticiones para obtener las proporciones. La distribución binomial teórica se superpone utilizando points y lines para mostrar cómo se ajustan los datos simulados a la distribución teórica.

Además, dbinom(x, size, prob) nos dice la probabilidad de obtener exactamente "x" éxitos en "size" pruebas, con cada prueba teniendo una probabilidad de éxito igual a "prob".

Para una moneda con probabilidad p=0.3 de obtener águila tenemos algo del este estilo:
...
```
La interpretación de todo esto sería ver como, dado que tenemos una probabilidad p=0.3 de obtener águilas, entonces la aparición de águilas en los volados serán menos frecuentes. Esto se ve claramente reflejado en La distribución teórica, pues refleja dicha tendencia al mostrar probabilidades más altas para obtener menos águilas y más bajas para obtener más águilas.
