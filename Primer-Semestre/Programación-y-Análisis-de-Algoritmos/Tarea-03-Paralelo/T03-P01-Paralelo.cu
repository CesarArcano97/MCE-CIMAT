...
int main() {
    ...
    auto startTotal = std::chrono::high_resolution_clock::now(); // Inicio del tiempo total

    // Medir tiempo de asignación y transferencia a GPU
    ...
    auto endTransferToGPU = std::chrono::high_resolution_clock::now();
    ...
    // Medir tiempo de transferencia de la GPU al CPU
    auto startTransferToCPU = std::chrono::high_resolution_clock::now();
    cudaMemcpy(imgC.data, d_C, imgSize, cudaMemcpyDeviceToHost);
    auto endTransferToCPU = std::chrono::high_resolution_clock::now();

    auto endTotal = std::chrono::high_resolution_clock::now(); // Fin del tiempo total

    // Guardar la imagen resultante
    cv::imwrite("/home/cesar/Documentos/Tareas-CIMAT/Primer_Semestre/PyAA/Programacion-Paralelo/output-paralelo.jpg", imgC);
    ...
    std::chrono::duration<float, std::milli> transferToGPU = endTransferToGPU - startTransferToGPU;
    std::chrono::duration<float, std::milli> kernelExecution = endKernel - startKernel;
    std::chrono::duration<float, std::milli> transferToCPU = endTransferToCPU - startTransferToCPU;
    std::chrono::duration<float, std::milli> totalTime = endTotal - startTotal;

    std::cout << "Tiempo de transferencia a GPU: " << transferToGPU.count() << " ms" << std::endl;
    std::cout << "Tiempo de ejecución del kernel: " << kernelExecution.count() << " ms" << std::endl;
    std::cout << "Tiempo de transferencia a CPU: " << transferToCPU.count() << " ms" << std::endl;
